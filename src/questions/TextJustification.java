...
public class TextJustification
{
	 ...
	 *         neatly
	 * 
	 *         Solution: Badness - We define badness has square of empty spaces
	 *         in every line. So 2 empty space on one line gets penalized as 4
	 *         (2^2) while 1 each empty space on 2 lines gets penalized as 2(1 +
	 *         1). So we prefer 1 empty space on different lines over 2 empty
	 *         space on one line.
	 * 
	 *         For every range i,j(words from i to j) find the cost of putting
	 *         them on one line. If words from i to j cannot fit in one line
	 *         cost will be infinite. Cost is calculated as square of empty
	 *         space left in line after fitting words from i to j.
	 * 
	 *         Then apply this formula to get places where words need to be
	 *         going on new line. minCost[i] = minCost[j] + cost[i][j-1] Above
	 *         formula will try every value of j from i to len and see which one
	 *         gives minimum cost to split words from i to len.
	 * 
